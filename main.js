...
// ================= 终极图片交互引擎 (Zoom + Pan + PreventClose) =================
  ...
  let imgState = {
    ...
    currX: 0,
    currY: 0,
    isDragging: false // 用于区分是点击还是拖拽
  };

  const resetState = () => {
    imgState = { scale: 1, panning: false, startX: 0, startY: 0, currX: 0, currY: 0, isDragging: false };
  ...
  // 监听 LightGallery 生命周期
  gallery.addEventListener('lgBeforeSlide', resetState);
  gallery.addEventListener('lgAfterClose', resetState);

  // 2. 核心功能挂载 (使用 capture: true 确保先于 LightGallery 执行)

  // [A] 滚轮缩放
  document.addEventListener('wheel', (e) => {
    const img = e.target.closest('.lg-current .lg-object');
    if (!img) return;
    ...
    updateTransform(img, true); // true = 启用动画
  }, { passive: false });
  ...
  document.addEventListener('mousedown', (e) => {
    const img = e.target.closest('.lg-current .lg-object');
    // 只有在放大状态下才允许拖拽
    if (!img || imgState.scale <= 1) return;

    e.preventDefault();
    imgState.panning = true;
    imgState.isDragging = false; // 重置拖拽标记
    imgState.startX = e.clientX - imgState.currX;
    imgState.startY = e.clientY - imgState.currY;

    img.style.cursor = 'grabbing';
    img.style.transition = 'none'; // 拖拽时关闭动画，保证跟手
  }, true);
  ...
  document.addEventListener('mousemove', (e) => {
    ...
    imgState.currX = e.clientX - imgState.startX;
    imgState.currY = e.clientY - imgState.startY;

    const img = document.querySelector('.lg-current .lg-object');
    if (img) updateTransform(img, false); // false = 无动画
  }, true);
  ...
  document.addEventListener('mouseup', (e) => {
    if (imgState.panning) {
      ...
      // 关键技巧：让 isDragging 状态稍微再存活一小会儿，
      // 以便让随后的 click 事件能检测到刚才发生了拖拽
      setTimeout(() => { imgState.isDragging = false; }, 50);
    }
  }, true);
  ...
  document.addEventListener('click', (e) => {
    ...
    // 如果点击的是图片区域
    if (target.closest('.lg-object') || target.closest('.lg-img-wrap')) {

      // 情况1：刚才进行了拖拽操作 -> 拦截 (防止拖完松手就关了)
      // 情况2：当前处于放大状态 -> 拦截 (防止在查看细节时误触关闭)
      if (imgState.isDragging || imgState.scale > 1) {
        e.preventDefault();
        e.stopPropagation();
        e.stopImmediatePropagation(); // 强力阻断
        console.log("拦截关闭：处于放大或拖拽状态");
        return false;
      }
      // 情况3：1倍状态下单纯点击 -> 放行 (LightGallery 默认会关闭，符合直觉)
    }
  }, true); // 必须在捕获阶段拦截

  // 辅助函数：应用样式
  function updateTransform(img, useTransition) {
    if (useTransition) {
      img.style.transition = 'transform 0.1s ease-out';
    }
    img.style.transform = `translate3d(${imgState.currX}px, ${imgState.currY}px, 0) scale(${imgState.scale})`;
  }
